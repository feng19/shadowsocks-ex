defmodule Shadowsocks.Encoder do
  ...
  def encode(
        %Encoder{type: :block, method: method, key: key, enc_iv: iv, enc_rest: rest} = encoder,
        data
      )
  ...
  defp encode_aead(state, data, len, res_data_acc) do
    {chunk_data, chunk_len, rest_data} =
      cond do
        len > 0x3FFF ->
          <<long_chunk_data::binary-size(0x3FFF), long_rest_data::binary>> =
    ...
    {enc_len, enc_len_tag} =
      :crypto.block_encrypt(
        ...
        key,
        aead_nonce(ctr + 0),
        {<<>>, chunk_len_bin, @aead_tag_len}
      )

    {enc_data, enc_data_tag} =
      :crypto.block_encrypt(method, key, aead_nonce(ctr + 1), {<<>>, chunk_data, @aead_tag_len})
  ...
  def decode(
        %Encoder{type: :aead, dec_stream: aead_state, dec_rest: {rest, rsize}} = encoder,
        data
      ) do
    data_size = IO.iodata_length(data)

    new_data =
      case {rsize, data_size} do
        {0, 0} -> []
        {0, _} -> data
        {_, 0} -> rest
        {_, _} -> [rest, data]
    ...
    {aead_state, {new_rest, new_rsize}, dec_data} =
      decode_aead(aead_state, new_data, rsize + data_size, [])

    {%Encoder{encoder | dec_stream: aead_state, dec_rest: {new_rest, new_rsize}},
     IO.iodata_to_binary(dec_data)}
  end

  def decode(
        %Encoder{type: :block, method: method, key: key, dec_iv: iv, dec_rest: {rest, rsize}} =
          encoder,
        data
      )
      when method in [:aes_128_cfb, :aes_192_cfb, :aes_256_cfb] do
  ...
  defp decode_aead(state, acc, _acc_size, res_data_acc) do
    ...
    <<enc_chunk_data::binary-size(chunk_data_size), chunk_tag::binary-size(@aead_tag_len),
      acc::binary>> = IO.iodata_to_binary(acc)
  ...
  defp decode_aead_chunk(
         %AEADState{chunk: {:frame_len, 2}} = state,
         <<0::2, frame_data_size::14>>
       ) do
  ...
  def encode_once(%Encoder{type: :aead, method: method, key: key, enc_iv: eiv}, data) do
    ...
    {enc_data, enc_data_tag} =
      :crypto.block_encrypt(aead_method, aead_key, aead_nonce(0), {<<>>, data})
  ...
  def decode_once(%Encoder{type: :aead, method: method, key: key, enc_iv: eiv}, data) do
    ...
    <<iv::binary-size(ivlen), enc_payload::binary-size(payload_size),
      tag::binary-size(@aead_tag_len)>> = data
