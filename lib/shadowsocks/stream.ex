defmodule Shadowsocks.Stream do
  ...
  *note* the size only can be 0 when OTA stream
  """
  def recv(
        %{ota: true, ota_id: id, ota_iv: iv, sock: sock, encoder: encoder} = stream,
        0,
        timeout
      ) do
    ...
    with {:ok, data1} <- wait_data(sock, 2, timeout),
         ...
         {encoder, <<hmac::binary-size(@hmac_len), data::binary>>} <-
           Shadowsocks.Encoder.decode(encoder, data2),
    ...
    with {:ok, data} <- wait_data(sock, size - acc_size, timeout),
      ...
      recv(
        %Shadowsocks.Stream{
          stream
          | encoder: encoder,
            recv_rest: {[acc, data], acc_size + byte_size(data)}
        },
        size
      )
    else
